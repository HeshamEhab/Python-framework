...
## Using AJAX with Multiple Classes in Your App

You can use AJAX not only with the main class but also with multiple classes across your app. All you need is to set up a pipeline like the following:

---

### Step 1: Create a JavaScript Method to Trigger AJAX

Define a helper function that takes the class name, method name, data, and a callback:

```javascript
function call(className, method, data, ret) { 
    data["get"] = method;
    data["class"] = className; 
    ins("_call")._ajax._app(data, ret);
}
```

This function allows you to call **any method** in **any class** on your app backend via AJAX.

---

### Step 2: Example Usage of the `call()` Function

Here's how to use the function to call a method inside a specific class:

```javascript
call("Search", "_products_ui", sdata, data => { 
    ins(".-products-area")._setHTML(data);
});
```

This line calls the `_products_ui` method inside the `AppProductsSearch` class.

---

### Step 3: Python Side – Handling the AJAX Request

In your main app class (Python), you need to handle the AJAX request and route it to the desired class and method:

```python
def _call_search(self):
    q = self.ins._server._post()
    return eval(f"AppProducts{q['class']}(self.app).{q['get']}()")
```

This method dynamically calls the appropriate method from the specified class using the data received from the AJAX request.

For example:
If `q['class']` is `"Search"` and `q['get']` is `"_products_ui"`, the function will execute:

```python
AppProductsSearch(self.app)._products_ui()
```
...
- Make sure the class you're calling (`AppProductsSearch` in this case) is defined and imported properly.
- Use caution when using `eval()` due to security implications. Only allow safe and verified inputs.
